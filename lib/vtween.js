...
class vtween{
    ...
    tick(t){
        ...
        let engineTime;
        if(this.direction){
            engineTime =(this.now - this.startTime);
        }else{
            engineTime =(this.lastTime -this.now -this.startTime);
        }
            
        let insTime=engineTime;
        ...
        if(insTime<0){
            ...
            if(!this.end){
                ...
                if(this.loop){
                    //loop
                    insTime=this.lastTime+insTime;
                    this.end=false;
                }
            }
        }

        if(insTime>=this.duration){
            //end
        ...
        const elapsed = minMaxValue(insTime -  this.delay, 0, this.duration) / this.duration;
